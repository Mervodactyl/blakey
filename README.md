...
The important thing is to choose a typescale, then do the calculations based around how they align to the baseline (or rather base font-size)
...
```
Finally calculate the __margin-bottom__. Most developers use the same calculations for the margin-bottom as they used for line-height with the exception that ```h1``` and ```h2``` may have half values, it depends on your preference really and the overall look f your web page
